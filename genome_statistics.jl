...
module PlotGenome
    ...
    function fun_plot_hits_histogram_layer!(plt, vec_str_chromosome_names, vec_int_chromosome_lengths, str_filename_coor;
        ...
        for i in 1:n
            ...
            vec_int_position = []
            while !eof(FILE)
                str_chrom, str_pos = split(readline(FILE), ',')[vec_idx_chr_pos]
    ...
    function fun_add_chords!(plt, str_filename_groupings_and_coordinates, vec_str_chromosome_names, vec_int_chromosome_lengths; delim=',', vec_idx_groups_chr_pos=[1,9,3,4], r=0.5, w=0.05, vec_colours="", linewidth=1, colour_per_chrom=false, add_legend=false, header=true)
        ...
        @showprogress for grp in vec_str_groups
            ...
            idx = X[:,1] .== grp
            Y = X[idx, :]
            # Count the number of seq per chr and find the next chr root with the most seq
            vec_int_chr_cnts = []
...
### EXECUTE ###
###############
function execute(str_filename_fasta, str_fname_output_svg, str_filename_LTR_COPIA, str_filename_LTR_GYPSY, vec_idx_chr_pos, str_filename_groupings_and_coordinates, vec_idx_groups_chr_pos, n, n_int_tick_length_bp, n_int_tick_label_size, n_int_chrom_name_size; recompute=false, cleanup=false)
    ...
    ### Base plotsave_temp_files_and_data
