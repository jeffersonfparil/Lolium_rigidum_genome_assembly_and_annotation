...
dat$CUMM_LENGTH = cumsum(as.numeric(dat$LENGTH))
dat$CUMM_COVER = as.numeric(dat$CUMM_LENGTH)*100 / genome_size
...
N50 = min(dat$LENGTH[(dat$CUMM_COVER>=50) & !is.na(dat$CUMM_COVER) & !is.infinite(dat$CUMM_COVER)]) #contigs with size >= to this N50 value - cover 50% of the assembly
